...
// FIX: Import KpiMeasureColumn type
import { Client, Project, KpiRecord, KpiMeasure, KpiMeasureMaster, KpiMeasureRow, KpiMeasureColumn, ItemMaster, User } from '../types';
    ...
    return (
        <Modal title="マスターから追加" onClose={onClose}>
            <div className="space-y-3 max-h-[60vh] overflow-y-auto">
                {masters.length > 0 ? (
                    masters.map(master => (
                        <div key={master.id} onClick={() => onSelect(master)} className="p-4 border border-slate-200 rounded-lg hover:bg-sky-50 hover:border-sky-500 cursor-pointer transition-colors">
                            <h4 className="font-semibold text-sky-700">{master.name}</h4>
                            <p className="text-xs text-slate-500 mt-1">{master.columns.length}列, {master.rows.length}項目</p>
                        </div>
                    ))
                ) : (
...
const KpiMeasureRowDetailPage: React.FC = () => {
    const { clientId, projectId, recordId, measureId, rowId } = useParams<{ clientId: string; projectId: string; recordId: string; measureId: string; rowId: string }>();
    const { currentUser } = useAuth();
    const navigate = useNavigate();
    
    const [subMeasures, setSubMeasures] = useState<KpiMeasure[]>([]);
    ...
    const [parentMeasure, setParentMeasure] = useState<KpiMeasure | null>(null);
    const [currentRow, setCurrentRow] = useState<KpiMeasureRow | null>(null);
    ...
    const fetchMasters = async () => {
        ...
        setMasters(mastersSnapshot.docs.map(doc => ({ ...doc.data(), id: doc.id } as KpiMeasureMaster)));
    };
    ...
    useEffect(() => {
        if (!currentUser || !clientId || !projectId || !recordId || !measureId || !rowId) {
        ...
        const fetchAllData = async () => {
            setIsLoading(true);
            try {
                // Fetch all parent context data for breadcrumbs and titles
                ...
                const parentMeasureDoc = await firestore.doc(getCollectionPath.kpiMeasureDoc(currentUser.uid, projectId, recordId, measureId)).get();
                setParentMeasure({ ...parentMeasureDoc.data(), id: parentMeasureDoc.id } as KpiMeasure);

                const currentRowDoc = await firestore.doc(getCollectionPath.kpiMeasureRowDoc(currentUser.uid, projectId, recordId, measureId, rowId)).get();
                setCurrentRow({ ...currentRowDoc.data(), id: currentRowDoc.id } as KpiMeasureRow);
        ...
        fetchAllData();
    }, [currentUser, clientId, projectId, recordId, measureId, rowId]);
    
    useEffect(() => {
        if (!currentUser || !projectId || !recordId || !measureId || !rowId) return;

        const subMeasuresPath = getCollectionPath.kpiMeasureRowSubMeasures(currentUser.uid, projectId, recordId, measureId, rowId);
        const q = firestore.collection(subMeasuresPath).orderBy("order", "asc");

        const unsubscribe = q.onSnapshot(snapshot => {
            const fetchedMeasures = snapshot.docs.map((doc, index) => ({ ...doc.data(), id: doc.id, order: doc.data().order ?? index } as KpiMeasure));
            setSubMeasures(fetchedMeasures);
        }, err => {
            setError("サブ施策データの読み込みに失敗しました。");
        });
        return () => unsubscribe();
    }, [currentUser, projectId, recordId, measureId, rowId]);

    const addSubMeasure = async (data: Omit<KpiMeasure, 'id'>) => {
        if (!currentUser || !projectId || !recordId || !measureId || !rowId) return;
        const subMeasuresPath = getCollectionPath.kpiMeasureRowSubMeasures(currentUser.uid, projectId, recordId, measureId, rowId);
        await firestore.collection(subMeasuresPath).add(data);
    };

    const handleAddBlankMeasure = async () => {
        await addSubMeasure({
            name: "新規サブ施策テーブル", order: subMeasures.length, projectId: projectId!, userId: currentUser!.uid, kpiRecordId: recordId!, masterId: null,
        });
    };
    ...
    const handleAddFromMaster = async (master: KpiMeasureMaster) => {
        ...
        const subMeasureData: Omit<KpiMeasure, 'id'> = { name: master.name, order: subMeasures.length, projectId, userId: currentUser.uid, kpiRecordId: recordId, masterId: master.id };
        const subMeasuresPath = getCollectionPath.kpiMeasureRowSubMeasures(currentUser.uid, projectId, recordId, measureId, rowId);
        const measureDocRef = await firestore.collection(subMeasuresPath).add(subMeasureData);
        
        if (master.rows && master.rows.length > 0) {
            const batch = firestore.batch();
            const rowsPath = getCollectionPath.kpiMeasureRowSubMeasureRows(currentUser.uid, projectId, recordId, measureId, rowId, measureDocRef.id);
    ...
    const handleDeleteSubMeasure = async (subMeasureId: string) => {
        if (!currentUser || !projectId || !recordId || !measureId || !rowId || !window.confirm("この施策テーブル全体を削除しますか？")) return;

        const batch = firestore.batch();
        const subMeasureDocRef = firestore.doc(getCollectionPath.kpiMeasureRowSubMeasureDoc(currentUser.uid, projectId, recordId, measureId, rowId, subMeasureId));

        const columnsPath = getCollectionPath.kpiMeasureRowSubMeasureColumns(currentUser.uid, projectId, recordId, measureId, rowId, subMeasureId);
        const columnsSnapshot = await firestore.collection(columnsPath).get();
        columnsSnapshot.docs.forEach(doc => batch.delete(doc.ref));

        const rowsPath = getCollectionPath.kpiMeasureRowSubMeasureRows(currentUser.uid, projectId, recordId, measureId, rowId, subMeasureId);
        const rowsSnapshot = await firestore.collection(rowsPath).get();
        rowsSnapshot.docs.forEach(doc => batch.delete(doc.ref));

        batch.delete(subMeasureDocRef);
        await batch.commit();
    };

    const handleDuplicateSubMeasure = async (subMeasureIdToDuplicate: string) => {
        if (!currentUser || !projectId || !recordId || !measureId || !rowId) return;
    
        const originalSubMeasure = subMeasures.find(m => m.id === subMeasureIdToDuplicate);
        if (!originalSubMeasure) {
            setError("複製元のサブ施策が見つかりませんでした。");
            return;
        }
        ...
        try {
            ...
            await copyMeasureDeep(
                ...
                { recordId, measureId: subMeasureIdToDuplicate, parentMeasureId: measureId, parentRowId: rowId },
                { recordId, newName: `${originalSubMeasure.name}のコピー`, newOrder: subMeasures.length, parentMeasureId: measureId, parentRowId: rowId }
            );
            await batch.commit();
        } catch (err: any) {
            console.error("Error duplicating sub-measure:", err);
            setError(err.message || "サブ施策の複製に失敗しました。");
        }
    };

    const handleCopyFromOtherRecord = async (sourceRecordId: string, sourceMeasureId: string) => {
        if (!currentUser || !projectId || !recordId || !measureId || !rowId) {
        ...
        await copyMeasureDeep(
            ...
            { recordId, newName: sourceMeasureData.name, newOrder: subMeasures.length, parentMeasureId: measureId, parentRowId: rowId }
        );
        await batch.commit();
        setShowCopyMeasureModal(false);
    };

    if (isLoading) return <LoadingSpinner text="施策詳細データを読み込み中..." />;
    if (error) return <div className="p-4 bg-red-100 text-red-700 rounded-md">{error}</div>;
    if (!client || !project || !kpiRecord || !parentMeasure || !currentRow) return <div className="p-4 bg-yellow-100 text-yellow-700 rounded-md">関連データが見つかりません。</div>;
        ...
        <div className="space-y-6">
            <nav aria-label="breadcrumb" className="text-sm text-slate-600 mb-2">
                <ol className="flex items-center space-x-1 flex-wrap">
                    ...
                    <li><ChevronRightIcon className="text-slate-400" /><button onClick={() => navigate(`/clients/${clientId}/projects/${projectId}/kpi/${recordId}`)} className="hover:text-sky-600 hover:underline">施策管理: {kpiRecord.periodLabel}</button></li>
                    <li><ChevronRightIcon className="text-slate-400" /><span className="font-medium text-slate-700">項目詳細: {currentRow.name}</span></li>
                </ol>
            </nav>

             <div className="bg-white p-6 rounded-lg shadow-md flex justify-between items-center">
                <div>
                    <h2 className="text-2xl font-semibold text-slate-800 mb-1">項目詳細: {currentRow.name}</h2>
                    <p className="text-sm text-slate-500">親施策: {parentMeasure.name} ({kpiRecord.periodLabel})</p>
                </div>
                <div className="flex items-center gap-2">
                     ...
                     <div className="relative group">
                         <PrimaryButton icon={<PlusIcon/>}>サブ施策テーブルを追加 <ChevronDownIcon className="group-hover:rotate-180 transition-transform"/></PrimaryButton>
            ...
            <div className="space-y-8">
                {subMeasures.map((subMeasure, index) => (
                    <div
                        key={subMeasure.id}
                    >
                        <MeasureTable
                            measure={subMeasure}
                            project={project}
                            onDelete={handleDeleteSubMeasure}
                            onDuplicate={handleDuplicateSubMeasure}
                            masters={masters}
                            onMastersUpdate={fetchMasters}
                            kpiRecord={kpiRecord}
                            allKpiRecords={sheetKpiRecords}
                            itemMasters={itemMasters}
                            parentRowContext={{ parentMeasureId: measureId, parentRowId: rowId }}
                        />
                    </div>
                ))}
                {subMeasures.length === 0 && (
                    <div className="text-center py-12 bg-white rounded-lg shadow">
                         <h3 className="mt-2 text-lg font-medium text-slate-900">サブ施策テーブルがありません</h3>
                        <p className="mt-1 text-sm text-slate-500">最初のサブ施策テーブルを追加して始めましょう。</p>
                    </div>
                )}
            </div>
            ...
            {showCopyMeasureModal && <CopyMeasureModal project={project} currentRecordId={recordId} allKpiRecords={allKpiRecords} onClose={() => setShowCopyMeasureModal(false)} onCopy={handleCopyFromOtherRecord} parentMeasureId={measureId} />}
        </div>
    );
};

export default KpiMeasureRowDetailPage;